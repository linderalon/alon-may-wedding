...
interface PollConfig {
  id: string;
  questionKey: string;
  candidates: readonly { id: string; labelKey: string; color: string; dark: string }[];
}

const POLLS: readonly PollConfig[] = [
  {
    id: "whoCries",
    questionKey: "polyQ1",
    candidates: [
  ...
  },
  {
    ...
    questionKey: "polyQ2",
    candidates: [
      { id: "one",         labelKey: "polyGlassOne",       color: "#D6A99D", dark: "#B8826C" },
      { id: "two",         labelKey: "polyGlassTwo",       color: "#9CAFAA", dark: "#5C8870" },
      { id: "three-plus",  labelKey: "polyGlassThreePlus", color: "#C4A882", dark: "#8C6C42" },
    ],
  },
  {
    id: "entranceSong",
    questionKey: "polyQ3",
    candidates: [
      { id: "rnb",             labelKey: "polySongRnb",    color: "#D6A99D", dark: "#B8826C" },
      { id: "heavy-metal",     labelKey: "polySongMetal",  color: "#9CAFAA", dark: "#5C8870" },
      { id: "classic-hebrew",  labelKey: "polySongHebrew", color: "#C4A882", dark: "#8C6C42" },
      { id: "middle-eastern",  labelKey: "polySongMizrahi",color: "#B8A0C8", dark: "#7858A0" },
    ],
  },
] as const;
...
const STORAGE_PREFIX = "poly-wedding-vote:";

function totalVotes(v: Votes) {
  return Object.values(v).reduce((s, n) => s + n, 0);
}
function pct(v: Votes, id: string) {
  const t = totalVotes(v);
  return t === 0 ? Math.round(100 / Object.keys(v).length) : Math.round((v[id] / t) * 100);
}

function PollBlock({ poll, allVotes, refetch }: {
  poll: PollConfig;
  allVotes: AllPolls | null;
  refetch: (updated: AllPolls) => void;
}) {
  ...
  const votes = allVotes?.[poll.id] ?? null;
  const totalCount = votes ? totalVotes(votes) : 0;
  ...
  React.useEffect(() => {
    const stored = localStorage.getItem(STORAGE_PREFIX + poll.id);
  ...
  const castVote = async (id: string) => {
    ...
    try {
      const res = await fetch("/api/votes", {
        ...
        body: JSON.stringify({ poll: poll.id, candidate: id }),
      });
      const updated: AllPolls = await res.json();
      refetch(updated);
      setVoted(id);
      setJustVoted(true);
      localStorage.setItem(STORAGE_PREFIX + poll.id, id);
  ...
  return (
    <div className="flex flex-col gap-4 sm:gap-5">
      <h3 className="font-[family-name:var(--font-display)] italic font-light text-lg sm:text-xl text-[#3D2B1F] text-center">
        {t[poll.questionKey as keyof typeof t] as string}
      </h3>
      ...
      <div className="flex flex-col gap-3 sm:gap-4">
        {sortedCandidates.map((c) => {
          ...
          const percentage = votes ? pct(votes, c.id) : Math.round(100 / candidates.length);
          const voteCount  = votes ? votes[c.id] : 0;

          return (
            <motion.div key={c.id} layout transition={{ type: "spring", stiffness: 260, damping: 26 }}>
              <button onClick={() => castVote(c.id)} disabled={hasVoted || loading}
...
export function PolyWedding() {
  ...
  const [allVotes, setAllVotes] = React.useState<AllPolls | null>(null);

  React.useEffect(() => {
    fetch("/api/votes")
      .then((r) => r.json())
      .then((data: AllPolls) => setAllVotes(data));
  }, []);
  ...
  return (
    <section id="poly-wedding" aria-label={t.polyHeading}
      ...
      {/* polls */}
      <div className="relative z-10 max-w-lg mx-auto flex flex-col gap-12 sm:gap-16">
        {POLLS.map((poll) => (
          <PollBlock key={poll.id} poll={poll} allVotes={allVotes} refetch={setAllVotes} />
        ))}
      </div>
    </section>
  );
